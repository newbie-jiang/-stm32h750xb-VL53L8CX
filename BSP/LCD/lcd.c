...
/*刷图函数*/
void LCD_DrawBitmap(int startX, int startY, int width, int height, const uint16_t* bitmap) {
    for(int y = 0; y < height; y++) {
        for(int x = 0; x < width; x++) {
            uint32_t color = (uint32_t)(*bitmap++); // 将16位颜色转换为32位
            lcd_draw_point(startX + x, startY + y, color);
        }
    }
}
