...
static void MX_53L8A1_SimpleRanging_Process(void)
{
  ...
  Profile.EnableAmbient = 0; /* Enable: 1, Disable: 0   环境光强度  */
  Profile.EnableSignal = 0; /* Enable: 1, Disable: 0    信号 */
...
static uint8_t get_key(void)
{
  ...
  HAL_UART_Receive(&huart4, &cmd, 1, HAL_MAX_DELAY);
...
static uint32_t com_has_data(void)
{
  return __HAL_UART_GET_FLAG(&huart4, UART_FLAG_RXNE);;
}
